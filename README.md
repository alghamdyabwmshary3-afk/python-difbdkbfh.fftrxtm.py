...
# إضافة معالج للطباعة على الشاشة أيضاً
...
def update_user_rank(user_id):
    ...
    try:
        ...
        logger.error(f"❌ خطأ في تحديث المستوى: {e}")
        return '👤 عضو'
...
def check_rate_limit(user_id, max_requests=30):
    ...
    try:
        ...
        logger.error(f"❌ خطأ في التحقق من الحدود: {e}")
        return True
...
def increment_download_stats(user_id, media_type, platform="unknown", file_size=0):
    ...
    try:
        ...
        logger.info(f"📥 تم تسجيل تحميل للمستخدم {user_id}: {media_type} من {platform}")
    except Exception as e:
...
def save_channel(channel_id, channel_title="غير معروف"):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info(f"📢 تم إضافة قناة جديدة: {channel_title}")
        ...
        logger.error(f"❌ خطأ في حفظ القناة: {e}")
...
def clear_channels():
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info("🗑️ تم تصفير القنوات الإجبارية")
    except Exception as e:
        logger.error(f"❌ خطأ في تصفير القنوات: {e}")
...
def save_feedback(user_id, feedback_text):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info(f"📝 تم استلام ملاحظة من المستخدم {user_id}")
    except Exception as e:
...
def update_daily_reward(user_id):
    ...
    try:
        ...
        logger.error(f"❌ خطأ في تحديث المكافأة: {e}")
...
def handle_callbacks(call):
    ...
    if call.data == "stats_advanced":
        ...
        stats_text = (
            ...
            f"💾 استخدام الذاكرة: `{memory_usage}`"
        )
    ...
    if call.data == "back_to_main":
        ...
        markup.add(
            ...
            InlineKeyboardButton("ℹ️ الميزات", callback_data="about_bot")
        )
        if chat_id == OWNER_ID:
            markup.add(
...
def daily_reward(message):
    ...
    if get_daily_reward(user_id):
        ...
        logger.info(f"🎁 تم منح المكافأة اليومية للمستخدم {user_id}")
    else:
...
def detect_channel_add(update):
    ...
    if update.chat.type == "channel" and update.new_chat_member.status in ["administrator", "creator"]:
        try:
            ...
            logger.error(f"Error detecting channel add: {e}")
...
# ============================================
# 📊 نظام التقارير اليومية التلقائية
# ============================================

def send_daily_report():
    """إرسال تقرير يومي للمطور"""
    while True:
        try:
            ...
            # تسجيل الإحصائيات اليومية في قاعدة البيانات
            conn = sqlite3.connect(DB_FILE)
            c = conn.cursor()
            today = datetime.now().strftime("%Y-%m-%d")
            ...
            c.execute('''INSERT INTO daily_stats (stat_date, total_users, active_users, total_downloads, new_users)
                         VALUES (?, ?, ?, ?, ?)''',
                      (today, total_users, active_today, total_downloads, new_users))
            conn.commit()
            conn.close()
            ...
            report = (
                ...
                f"💾 استخدام الذاكرة: `{memory_usage}`\n"
                "━━━━━━━━━━━━━━━━━━━\n"
                "✅ **البوت يعمل بكفاءة واستقرار**"
            )
            
            bot.send_message(OWNER_ID, report, parse_mode="Markdown")
            logger.info("📊 تم إرسال التقرير اليومي للمطور")
        ...
        try:
            ...
            if os.path.exists(DB_FILE):
                ...
                with zipfile.ZipFile(f"{backup_name}.zip", 'w') as zipf:
                    zipf.write(backup_name)
                
                os.remove(backup_name)
                
                # إرسال للمطور
                with open(f"{backup_name}.zip", 'rb') as f:
                    bot.send_document(OWNER_ID, f, caption="💾 نسخة احتياطية لقاعدة البيانات")
                
                os.remove(f"{backup_name}.zip")
                logger.info(f"✅ تم عمل نسخة احتياطية: {backup_name}")
        except Exception as e:
...
def auto_cleanup():
    """تنظيف الملفات المؤقتة والذاكرة بشكل دوري"""
    while True:
        try:
            # تنظيف مجلد التحميلات كل ساعة
            time.sleep(3600)
                    ...
                    try:
                        if os.path.isfile(file_path):
                            ...
                            if file_age > 1800:  # 30 دقيقة
                                os.remove(file_path)
                                logger.info(f"🧹 تم حذف ملف قديم: {file}")
                        elif os.path.isdir(file_path):
                            shutil.rmtree(file_path, ignore_errors=True)
                    except:
                        pass
            
            # تنظيف جلسات المستخدمين القديمة كل ساعتين
            if len(user_sessions) > 100:
                current_time = time.time()
                keys_to_delete = []
                # إضافة طابع زمني للجلسات
                for key, value in user_sessions.items():
                    if isinstance(value, dict) and value.get('timestamp', 0) < current_time - 3600:
                ...
                if keys_to_delete:
                    logger.info(f"🧹 تم تنظيف {len(keys_to_delete)} جلسة قديمة")
                
        except Exception as e:
            logger.error(f"❌ خطأ في التنظيف: {e}")

# ============================================
# ❤️ نظام إبقاء البوت حياً (Keep-Alive)
# ============================================

def keep_alive():
    """إبقاء البوت حياً عبر إرسال إشارات دورية"""
    while True:
        try:
            # التحقق من اتصال الإنترنت
            requests.get("https://api.telegram.org", timeout=5)
            
            # إرسال نبض حياة للمطور كل 12 ساعة
            time.sleep(43200)  # 12 ساعة
            bot.send_message(OWNER_ID, "❤️ **البوت يعمل بكفاءة وبشكل مستقر**")
            
        except Exception as e:
            logger.error(f"⚠️ مشكلة في الاتصال: {e}")
            time.sleep(10)
...
print("📝 نظام السجلات: Logging System")
...
print("💾 نظام النسخ الاحتياطي: Auto Backup Daily")
print("📊 نظام التقارير: Daily Reports")
print("🔄 نظام إعادة التشغيل: Auto-Restart")
print("❤️ نظام المراقبة: Keep-Alive")
print("🔒 الخصوصية: جميع المصادر مخفية تماماً")
print("=" * 60)

logger.info("=" * 60)
logger.info("👑 تم إطلاق البوت الإمبراطوري المتطور بنجاح 100%!")
logger.info("📊 نظام التشغيل: SQLite Database")
logger.info("🛡️ نظام الحماية: Rate Limiting (30 طلب/يوم)")
logger.info("📢 نظام الاشتراك الإجباري: متطور مع تحقق فوري")
logger.info("🎁 نظام المكافآت اليومية: Daily Rewards")
logger.info("📝 نظام الملاحظات: Feedback System")
logger.info("👑 نظام المستويات: Ranks حسب التحميلات")
logger.info("💾 نظام النسخ الاحتياطي: Auto Backup Daily")
logger.info("📊 نظام التقارير: Daily Reports")
logger.info("🔄 نظام إعادة التشغيل: Auto-Restart")
logger.info("❤️ نظام المراقبة: Keep-Alive")
logger.info("=" * 60)
